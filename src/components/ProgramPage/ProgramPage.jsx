...
const ProgramPage = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setHover(Math.round(res.data.program.rating));
        setsimilarPrograms(res.data.index_programs);
  ...
  return (
    <React.Fragment>
      ...
      <section className='home-features similar-programs'>
        <h1>Similar Programs</h1>
        <div className="similar-program-container">

            {similarPrograms.map((program) => (
              <div key={program._id} className="similar-program-card">
          <h2 className="similar-program-title">{program.programName}</h2>
          <p>{program.numberSubscriptions} subscriptions</p>
          <Button type='link'>View Program</Button>
        </div>
            ))}
